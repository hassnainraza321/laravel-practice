@extends('index')
@section('title', Helper::getSiteTitle('Templates'))
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body no-lt-rt-pad">
                @if($is_found)
                    {!! Helper::getDatatables(['Name', 'Category', 'Status', 'Type', 'Health', 'Created At', 'Action']) !!}
                @else
                    <div class="text-center mt-2 mb-5">
                        <img src="{{ asset('images/no-cat.svg') }}">
                        <h4>Add templates to start broadcast.</h4>
                        <p>Use collections to start broadcast or whatsapp marketing.</p>
                        <a href="{{ route('template.create') }}" class="btn btn-sm btn-custom me-1">Create template</a>
                        <a href="{{ route('pre.approved.template') }}" class="btn btn-sm btn-custom me-1">Pre-approved template</a>
                    </div>
                @endif
            </div>
        </div>
...
<script type="text/javascript">
    ...
    let user_role = {{ auth()->user()->role ?? 'null' }};
    ...
    function createTable()
    {
        ...
        table = $('.datatable').DataTable({
                    drawCallback: function() {
                        ...
                        {
                            let button_html = user_role === 1 ? `<button type="button" class="btn btn-sm btn-success me-1 d-none remove-all-button" data-text="You want to send in pre-approved template!" data-confirmbuttontext="Yes, Send it!" data-icon="info" data-url="{{ route('pre.approved.template') }}">Send To Pre-Approved</button><button type="button" class="btn btn-sm btn-danger me-1 d-none remove-all-button"><i class="ri-delete-bin-5-line"></i></button>` : `<button type="button" class="btn btn-sm btn-danger me-1 d-none remove-all-button"><i class="ri-delete-bin-5-line"></i></button>`;

                            $('.dt-search').prepend(button_html);
                        }
                    },
                    "pageLength": 10,
                    "scrollX": false,
                    ...
                    "columns":[
                        ...
                        { "data": "type_name", "name":"filter_index" },
                        { "data": "health", "name":"filter_index" },
