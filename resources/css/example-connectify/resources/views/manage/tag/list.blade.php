@extends('index')
@section('title', Helper::getSiteTitle('Tags'))
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-body no-lt-rt-pad">
                @if($is_found)
                    {!! Helper::getDatatables(['Tag Name', 'Category', 'Customer Journey', 'First Message', 'Created At', 'Action']) !!}
                @else
                    <div class="text-center mt-2 mb-5">
                        <img src="{{ asset('images/no-cat.svg') }}">
                        <h4>Add tags to link with contacts.</h4>
                        <p>Use collections to start broadcast or whatsapp marketing.</p>
                        <a data-bs-toggle="modal" data-bs-target="#tag-modal-add" class="btn btn-sm btn-custom me-1">Create tag</a>
                        <button type="button" class="btn btn-sm btn-light me-1" data-bs-toggle="modal" data-bs-target="#tags-import">Import</button>
                    </div>
                @endif
            </div>
        </div>
...
@include('manage.tag.modals.import-tags')
@include('manage.tag.modals.add')
@endsection

@section('meta')
<meta name="class-to-open" content="tags">
<meta name="remove-url" content="{{ route('tags.remove') }}">
@endsection
    ...
    function createTable()
    {
        ...
        table = $('.datatable').DataTable({
                    ...
                    "layout": {
                        ...
                        topEnd: function () {
                            return `<div class="dt-btns">
                                        <a href="{{ route('tags.export') }}" class="btn btn-sm btn-light me-1">Export</a>
                                        <button type="button" class="btn btn-sm btn-light me-1" data-bs-toggle="modal" data-bs-target="#tags-import">Import</button>
                                        <a data-bs-toggle="modal" data-bs-target="#tag-modal-add" class="btn btn-sm btn-custom me-1">Create Tag</a>
                                            ...
                                            <div class="dropdown-menu sorting-filter-menu" aria-labelledby="dropdownMenuButton">
                                                ...
                                                <div class="dropdown-item">
                                                    <div class="form-check form-checkbox-dark">
                                                        <input type="radio" id="customRadio1" name="sort_by" value="title-asc" class="form-check-input">
                                                        <label class="form-check-label" for="customRadio1">Name (A to Z)</label>
                                                    </div>
                                                </div>
                                                <div class="dropdown-item">
                                                    <div class="form-check form-checkbox-dark">
                                                        <input type="radio" id="customRadio2" name="sort_by" value="title-desc" class="form-check-input">
                    ...
                    "columnDefs": [
                        ...
                        { className: "text-left", "targets": [ 2 ], "orderable": "false" },
                    ...
                    "columns":[
                        ...
                        { "data": "category_name", "name":"filter_index" },
                        { "data": "customer_journey", "name":"filter_index" },
                        { "data": "first_message", "name":"filter_index" },
